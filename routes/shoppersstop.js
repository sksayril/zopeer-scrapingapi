...
const ShoppersStopScraper = require('../utilities/shoppersstopScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new ShoppersStopScraper();
    ...
    try {
        ...
        const filename = `shoppersstop-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/shoppersstop/scrape?url=https://www.shoppersstop.com/the-body-shop-tea-tree-face-oil-10-ml/p-SS24TBS334383_base/colorChange/SS24TBS334383_base_NoColour" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new ShoppersStopScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'shoppershop.html');
