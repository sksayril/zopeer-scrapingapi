...
const ZeptoScraper = require('../utilities/zeptoScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new ZeptoScraper();
    ...
    try {
        ...
        const filename = `zepto-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/zepto/scrape?url=https://www.zeptonow.com/pn/agaro-dual-channel-tens-massager" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new ZeptoScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'zepto.html');
