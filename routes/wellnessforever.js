...
router.post('/scrape', async (req, res) => {
    ...
    try {
        ...
        const filename = `wellnessforever-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/wellnessforever/scrape?url=https://www.wellnessforever.com/product/pediasure-pwdr-vanila-box-950gm" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Wellness Forever scraper test successful", data: result });
    } catch (error) {
        console.error('Wellness Forever scraper test failed:', error);
        res.status(500).json({ success: false, message: "Wellness Forever scraper test failed", error: error.message });
    }
});
