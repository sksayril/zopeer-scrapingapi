...
const UrbanicScraper = require('../utilities/urbanicScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new UrbanicScraper();
    ...
    try {
        ...
        const filename = `urbanic-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/urbanic/scrape?url=https://in.urbanic.com/details/printed-windbreaker-jacket-1549292?vid=308&skcPGs=257741-356277" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new UrbanicScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'urnibac.html');
