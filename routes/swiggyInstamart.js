...
const SwiggyInstamartScraper = require('../utilities/swiggyInstamartScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new SwiggyInstamartScraper();
    ...
    try {
        ...
        const filename = `swiggy-instamart-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/swiggy-instamart/scrape?url=https://www.swiggy.com/instamart/item/BV2BWK845T" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new SwiggyInstamartScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'swiggyinstamert.html');
