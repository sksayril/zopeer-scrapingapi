...
const DmartScraper = require('../utilities/dmartScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new DmartScraper();
    ...
    try {
        ...
        const filename = `dmart-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/dmart/scrape?url=https://www.dmart.in/product/dettol-antiseptic-liquid-pantiseptic0dett1xx170222" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new DmartScraper();
        const htmlContent = await fs.readFile(path.join(__dirname, '../scrapinghtml/dmart.html'), 'utf8');
        const result = await scraper.scrapeProduct(htmlContent);
        result.url = 'https://www.dmart.in/product/dettol-antiseptic-liquid-pantiseptic0dett1xx170222';
        res.json({ success: true, message: "DMart scraper test successful", data: result });
    } catch (error) {
        console.error('DMart scraper test failed:', error);
        res.status(500).json({ success: false, message: "DMart scraper test failed", error: error.message });
    }
});
