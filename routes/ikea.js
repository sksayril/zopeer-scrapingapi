...
const IkeaScraper = require('../utilities/ikeaScraper');
...
router.post('/scrape', async (req, res) => {
    ...
    try {
        ...
        const filename = `ikea-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/ikea/scrape?url=https://www.ikea.com/in/en/p/korken-jar-with-lid-clear-glass-30213547/" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new IkeaScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'ikea.html');
