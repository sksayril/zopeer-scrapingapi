...
const PepperfryScraper = require('../utilities/pepperfryScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new PepperfryScraper();
    ...
    try {
        ...
        const filename = `pepperfry-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/pepperfry/scrape?url=https://www.pepperfry.com/product/stalley-sheesham-wood-arm-chair" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new PepperfryScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'pepperfry.html');
