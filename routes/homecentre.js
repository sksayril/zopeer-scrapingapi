...
const HomeCentreScraper = require('../utilities/homecentreScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new HomeCentreScraper();
    ...
    try {
        ...
        const filename = `homecentre-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/homecentre/scrape?url=https://www.homecentre.in/in/en/Decor/Home-Accessories/Clocks/HOMECENTRE-Casablanca-Table-Clock--5cm/p/1000012555990" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new HomeCentreScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'homecenter.html');
