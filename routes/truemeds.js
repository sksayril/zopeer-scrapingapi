...
const TruemedsScraper = require('../utilities/truemedsScraper');
...
router.post('/scrape', async (req, res) => {
    const { url } = req.body;
    const scraper = new TruemedsScraper();
    ...
    try {
        ...
        const filename = `truemeds-product-${timestamp}.json`;
...
router.get('/scrape', async (req, res) => {
    ...
    if (!url) {
        return res.status(400).json({ 
            success: false, 
            message: "Please provide a URL parameter. Example: /api/truemeds/scrape?url=https://www.truemeds.in/otc/soft-soles-gel-30-gm-tm-geel1-001716" 
        });
    }
...
router.get('/test', async (req, res) => {
    try {
        const scraper = new TruemedsScraper();
        const htmlFilePath = path.join(__dirname, '../scrapinghtml', 'trumeds.html');
